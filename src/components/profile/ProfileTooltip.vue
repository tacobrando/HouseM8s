<template>
  <Tooltip @update:toggle="logoutConfirm.update" :isOpen="isOpen" class="w-full md:w-[400px] right-0 md:right-4 top-[3.45rem] p-3">
    <div @click="navigate" class="p-1 cursor-pointer mb-2 border flex items-center justify-start rounded-lg shadow-md">
      <span class="flex hover:bg-gray-100 transition-all w-full p-3 rounded-md items-center">
        <ProfileAvatar mode="tooltip" class="w-10 h-10" />
        <ProfileName :username="user.userInfo.username" class="mx-2 text-black" />
      </span>
    </div>
    <button
      @click="logoutConfirm.toggle"
      class="w-full flex p-3 hover:bg-gray-100 rounded-md items-center"
    >
